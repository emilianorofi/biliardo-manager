...
export async function GET(){
 const [sourcePlayers,sourceClubs,sourceLeagues]=await Promise.all([
  ...
  prisma.league.findMany({
   where:{season:{status:{in:["PREPARATION","ACTIVE"]}}},
   orderBy:[{season:{number:"desc"}},{level:"asc"},{groupCode:"asc"}],
   select:{seasonId:true,level:true,groupCode:true,entries:{select:{clubId:true}}}
  })
 ]);
 const seasons=20,runs=20,seed=26092026,target=sourcePlayers.length;
 const clubs=new Map(sourceClubs.map(c=>[c.id,c]));
 const latestSeasonId=sourceLeagues[0]?.seasonId??null;
 const initialMemberships=new Map();
 for(const league of sourceLeagues.filter(l=>l.seasonId===latestSeasonId)){
  for(const entry of league.entries)initialMemberships.set(entry.clubId,{level:league.level,groupCode:league.groupCode});
 }
 const buckets=Array.from({length:seasons},()=>[]);
 const levelBuckets=Array.from({length:seasons},()=>({1:[],2:[],3:[],4:[]}));
 const entrantBuckets=Array.from({length:seasons},()=>[]);
 for(let run=0;run<runs;run++){
  const rnd=rng(seed+run*7919); let sid=-1;
  let players=sourcePlayers.map(p=>({...p,sourceId:p.id}));
  let memberships=new Map([...initialMemberships.entries()].map(([clubId,v])=>[clubId,{...v}]));
  for(let season=1;season<=seasons;season++){
   ...
   const byLevel=levelSnapshots(players,memberships);
   for(const level of [1,2,3,4])levelBuckets[season-1][level].push(byLevel[level]);
   entrantBuckets[season-1].push({
    count:entrants.length,
    avgOverall:avg(entrants.map(overall)),
    avgTalent:avg(entrants.map(p=>p.talent))
   });
   memberships=advancePyramid(players,memberships,rnd);
  }
 }
 ...
 const pyramid=levelBuckets.map((bucket,i)=>({
  season:i+1,
  levels:Object.fromEntries([1,2,3,4].map(level=>{
   const rows=bucket[level];
   const out={};
   for(const k of Object.keys(rows[0]??{}))out[k]=round(avg(rows.map(r=>r[k])),2);
   return [level,out];
  }))
 }));
 const entrants=entrantBuckets.map((bucket,i)=>({
  season:i+1,
  count:round(avg(bucket.map(r=>r.count)),2),
  avgOverall:round(avg(bucket.map(r=>r.avgOverall)),2),
  avgTalent:round(avg(bucket.map(r=>r.avgTalent)),2)
 }));
...
function specialtyWeights(s){
 if(!s)return Object.fromEntries(SKILLS.map(k=>[k,1]));
 const core=s==="ITALIANA"?new Set(["precisione","diretto"]):s==="GORIZIANA"?new Set(["precisione","sponde"]):new Set(["diretto","sponde"]);
 const comp=new Set(["tattica","realizzazione","misura"]);
 const rest=SKILLS.filter(k=>!core.has(k)&&!comp.has(k));
 const total=9, out={};
 for(const k of SKILLS)out[k]=core.has(k)?total*.5/core.size:comp.has(k)?total*.3/comp.size:total*.2/rest.length;
 return out;
}
function grow(p,v,s=null){const w=specialtyWeights(s);for(const k of SKILLS)p[k]=round(p[k]+v*w[k]*growthMultiplier(p[k]),3)}
...
function levelSnapshots(players,memberships){
 const result={};
 for(const level of [1,2,3,4]){
  const clubIds=new Set([...memberships.entries()].filter(([,m])=>m.level===level).map(([id])=>id));
  const levelPlayers=players.filter(p=>p.clubId!=null&&clubIds.has(p.clubId));
  const clubStrengths=[...clubIds].map(id=>clubStrength(players,id));
  result[level]={
   clubs:clubIds.size,
   players:levelPlayers.length,
   avgOverall:avg(levelPlayers.map(overall)),
   top10Average:avg([...levelPlayers].sort((a,b)=>overall(b)-overall(a)).slice(0,10).map(overall)),
   avgAge:avg(levelPlayers.map(p=>p.age)),
   avgClubStrength:avg(clubStrengths),
   bestClubStrength:Math.max(...clubStrengths,0),
   worstClubStrength:clubStrengths.length?Math.min(...clubStrengths):0
  };
 }
 return result;
}
function clubStrength(players,clubId){
 const roster=players.filter(p=>p.clubId===clubId).sort((a,b)=>overall(b)-overall(a));
 const starters=roster.slice(0,3);
 return starters.length?avg(starters.map(overall)):0;
}
function advancePyramid(players,memberships,rnd){
 const groups=new Map();
 for(const [clubId,m] of memberships){
  const key=m.level+":"+m.groupCode;
  const arr=groups.get(key)??[];
  arr.push(clubId); groups.set(key,arr);
 }
 const rankedGroups=new Map();
 for(const [key,clubIds] of groups){
  rankedGroups.set(key,[...clubIds].sort((a,b)=>{
   const sa=clubStrength(players,a)+gaussian(rnd)*2.25;
   const sb=clubStrength(players,b)+gaussian(rnd)*2.25;
   return sb-sa;
  }));
 }
 const next=new Map([...memberships.entries()].map(([id,m])=>[id,{...m}]));
 const groupCodes={1:["A"],2:["A","B"],3:["A","B","C","D"],4:["A","B","C","D","E","F","G","H"]};
 for(let level=1;level<=3;level++){
  const parents=groupCodes[level],children=groupCodes[level+1];
  parents.forEach((parent,parentIndex)=>{
   const parentTable=rankedGroups.get(level+":"+parent)??[];
   const childA=children[parentIndex*2],childB=children[parentIndex*2+1];
   const childATable=rankedGroups.get((level+1)+":"+childA)??[];
   const childBTable=rankedGroups.get((level+1)+":"+childB)??[];
   const relegatedA=parentTable[6],relegatedB=parentTable[7],promotedA=childATable[0],promotedB=childBTable[0];
   if(relegatedA!=null)next.set(relegatedA,{level:level+1,groupCode:childA});
   if(relegatedB!=null)next.set(relegatedB,{level:level+1,groupCode:childB});
   if(promotedA!=null)next.set(promotedA,{level,groupCode:parent});
   if(promotedB!=null)next.set(promotedB,{level,groupCode:parent});
  });
 }
 return next;
}

...
export async function GET(){
 ...
 for(let run=0;run<runs;run++){
  ...
  for(let season=1;season<=seasons;season++){
   ...
   for(let i=0;i<target-survivors.length;i++){
    ...
    const entrant=graduate(sid--,clubId,clubs.get(clubId)?.youthCoachLevel??1,rnd);
    entrant.birthSeason=season;
    entrant.milestones={60:null,70:null,80:null,90:null,peakOverall:overall(entrant),peakAge:entrant.age,retiredAge:null};
    entrants.push(entrant);
    cohorts.push(entrant);
   }
   for(const p of retired){
    if(p.milestones && p.milestones.retiredAge==null)p.milestones.retiredAge=p.age;
   }
   players=[...survivors,...entrants];
   for(const p of players){
    if(!p.milestones)continue;
    const o=overall(p);
    if(o>p.milestones.peakOverall){p.milestones.peakOverall=o;p.milestones.peakAge=p.age}
    for(const threshold of [60,70,80,90]){
      if(p.milestones[threshold]==null && o>=threshold)p.milestones[threshold]=p.age;
    }
   }
 ...
 const matureCohorts=cohorts.filter(p=>p.birthSeason<=10);
 const lifecycle={
  entrants:cohorts.length,
  avgStartOverall:round(avg(cohorts.map(overall)),2),
  avgTalent:round(avg(cohorts.map(p=>p.talent)),2),
  reached60:rate(matureCohorts,p=>p.milestones?.[60]!=null),
  reached70:rate(matureCohorts,p=>p.milestones?.[70]!=null),
  reached80:rate(matureCohorts,p=>p.milestones?.[80]!=null),
  reached90:rate(matureCohorts,p=>p.milestones?.[90]!=null),
  avgAge60:round(avg(matureCohorts.map(p=>p.milestones?.[60]).filter(v=>v!=null)),2),
  avgAge70:round(avg(matureCohorts.map(p=>p.milestones?.[70]).filter(v=>v!=null)),2),
  avgAge80:round(avg(matureCohorts.map(p=>p.milestones?.[80]).filter(v=>v!=null)),2),
  avgAge90:round(avg(matureCohorts.map(p=>p.milestones?.[90]).filter(v=>v!=null)),2),
  avgPeakOverall:round(avg(matureCohorts.map(p=>p.milestones?.peakOverall??0)),2),
  avgPeakAge:round(avg(matureCohorts.map(p=>p.milestones?.peakAge??0)),2),
  avgRetirementAge:round(avg(matureCohorts.map(p=>p.milestones?.retiredAge).filter(v=>v!=null)),2),
  retiredBy20:rate(matureCohorts,p=>p.milestones?.retiredAge!=null)
 };
...
function rate(values,predicate){return values.length?round(values.filter(predicate).length/values.length*100,2):0}

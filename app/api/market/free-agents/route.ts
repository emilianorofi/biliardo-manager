...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body !== "object" || body === null) {
      return NextResponse.json({ error: "Dati dell'ingaggio non validi." }, { status: 400 });
    }

    const listingId = Number((body as { listingId?: unknown }).listingId);
    if (!Number.isInteger(listingId) || listingId <= 0) {
      return NextResponse.json({ error: "Svincolato selezionato non valido." }, { status: 400 });
    }
    ...
    const result = await prisma.$transaction(async (transaction) => {
      ...
      if (lockedClub.length === 0) {
        throw new FreeAgentSigningError("Club principale non disponibile.", 404);
      }
      ...
      if (lockedListing.length === 0) {
        throw new FreeAgentSigningError("Lo svincolato selezionato non è disponibile.", 404);
      }
      ...
      if (!club || !listing) {
        throw new FreeAgentSigningError("Dati dell'ingaggio non disponibili.", 404);
      }
      if (
        ...
        throw new FreeAgentSigningError("Questo giocatore non è più svincolato.", 409);
      }
      ...
      if (club.balance < 0) {
        throw new FreeAgentSigningError(
          "Con saldo negativo non puoi ingaggiare nuovi giocatori.",
          400
        );
      }
      ...
      await transaction.$executeRaw`
        ...
        SET "economyAdjustedAt" = ${completedAt}
        WHERE "id" = ${listingId}
      `;
    ...
    return NextResponse.json(
      {
        message: `${result.playerName} è stato ingaggiato. Nessun costo immediato; stipendio settimanale ${formatCurrency(result.salary)}.`,
        signing: result,
      },
      { status: 201 }
    );
    ...
    console.error("Errore durante l'ingaggio dello svincolato:", error);
    return NextResponse.json({ error: "Impossibile completare l'ingaggio." }, { status: 500 });
  }
}
